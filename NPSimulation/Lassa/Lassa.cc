...
Lassa::Lassa(){
  ...
  // Dark Grey
  m_SiliconVisAtt = new G4VisAttributes(G4Colour(0.0,1.0,1.0));
  //m_SiliconVisAtt = new G4VisAttributes(G4Colour(0.3, 0.3, 0.3)) ;
  // Green
  m_CsIVisAtt = new G4VisAttributes(G4Colour(0.0, 1.0, 0.0)) ;
...
void Lassa::ReadSensitive(const G4Event* event){
 ...
 /* G4THitsMap<G4double*>* ThinSiHitMap;
  ...
  G4int ThinSiCollectionID = G4SDManager::GetSDMpointer()->GetCollectionID("Lassa_ThinSiScorer/ThinSi");
  ...
  for (ThinSi_itr = ThinSiHitMap->GetMap()->begin() ; ThinSi_itr != ThinSiHitMap->GetMap()->end() ; ThinSi_itr++){
    ...
    if(E_ThinSi>EnergyThreshold){
        m_EventLassa->SetLassaThinSiStripEEnergy(E_ThinSi);
      	m_EventLassa->SetLassaThinSiStripEDetectorNbr(Info[7]);
        m_EventLassa->SetLassaThinSiStripEStripNbr(Info[8]);
	
        m_EventLassa->SetLassaThinSiStripTTime(Info[1]);
        m_EventLassa->SetLassaThinSiStripTDetectorNbr(Info[7]);
        m_EventLassa->SetLassaThinSiStripTStripNbr(Info[8]);
    }
  }
...
*/
  ...
  for (ThickSi_itr = ThickSiHitMap->GetMap()->begin() ; ThickSi_itr != ThickSiHitMap->GetMap()->end() ; ThickSi_itr++){
    ...
    if(E_ThickSi>EnergyThreshold){
        ...
        ms_InterCoord->SetDetectedPositionX(Info[2]) ; //ms_InterCoord->SetDetectedPositionX(Info[2]) ;
        ms_InterCoord->SetDetectedPositionY(Info[3]) ; //ms_InterCoord->SetDetectedPositionY(Info[3]) ;
        ms_InterCoord->SetDetectedPositionZ(Info[4]) ;
        ms_InterCoord->SetDetectedAngleTheta(Info[5]/deg) ; //ms_InterCoord->SetDetectedAngleTheta(Info[5]/deg) ;
        ms_InterCoord->SetDetectedAnglePhi(Info[6]/deg) ;//ms_InterCoord->SetDetectedAnglePhi(Info[6]/deg) ;
    }
  }//-1 is so that the strip,crystal and detector numbers start at 0, energies and times should not have a -1
  // Clear Map for next event
  ThickSiHitMap->clear();
...
void Lassa::InitializeScorers(){
 ...
 // m_ThinSiStripScorer = CheckScorer("Lassa_ThinSiScorer",already_exist);
