...
void Microball::ReadConfiguration(string Path){
    ...
    while (!ConfigFile.eof()) {
        ...
        while(ReadingStatus){
            ...
            else if (DataBuffer.compare(0, 13, "CRYSTALS_DONE") == 0) {
                check_Array = true;
            }
...
void Microball::ConstructDetector(G4LogicalVolume* world){
    ...
    /*G4double ring1Bool = 1;
     G4double ring2Bool = 1;
     G4double ring3Bool = 1;
     G4double ring4Bool = 1;
     G4double ring5Bool = 1;
     G4double ring6Bool = 1;
     G4double ring7Bool = 1;
     G4double ring8Bool = 1;
     G4double ring9Bool = 1;*/
    ...
    if(m_Ring1){
        ...
        G4double crystal_rmaxa1 = 0.0001*mm;
        ...
        for(int i = 1; i <= detNum1; i++)
        {
            ...
            G4ThreeVector d0 = G4ThreeVector(distanceFromTarget1*sin(coneangle1)*cos(0.5*360*deg/detNum1),
                                             distanceFromTarget1*sin(coneangle1)*sin(0.5*360*deg/detNum1),
                                             distanceFromTarget1*cos(coneangle1));
            G4ThreeVector df = G4ThreeVector(distanceFromTarget1*sin(theta1)*sin(i*360*deg/detNum1+.001*deg/*+0.5*360*deg/detNum1*/),
                                             distanceFromTarget1*sin(theta1)*cos(i*360*deg/detNum1+.001*deg/*+0.5*360*deg/detNum1*/),
            ...
            pqr.setRows(p,q,r);
            lg_rotm.rotateZ(i*360*deg/detNum1);
            ...
            G4ThreeVector w = G4ThreeVector(sin(theta1+90*deg)*sin(i*360*deg/detNum1+.001*deg), sin(theta1+90*deg)*cos(i*360*deg/detNum1+.001*deg), -cos(theta1+90*deg));
            ...
            delrin_rotm.rotateZ((-i)*360*deg/detNum1+.001*deg);
    ...
    if(m_Ring2){
        ...
        G4double crystal_rmaxa2 = 0.0001*mm;
        ...
        for(int i = 1; i <= detNum2; i++)
        {
            ...
            G4ThreeVector d0 = G4ThreeVector(distanceFromTarget2*sin(coneangle2)*cos(0.5*360*deg/detNum2),
                                             distanceFromTarget2*sin(coneangle2)*sin(0.5*360*deg/detNum2),
                                             distanceFromTarget2*cos(coneangle2));
            G4ThreeVector df = G4ThreeVector(distanceFromTarget2*sin(theta2)*sin(i*360*deg/detNum2+.001*deg/*+0.5*360*deg/detNum2*/),
                                             distanceFromTarget2*sin(theta2)*cos(i*360*deg/detNum2+.001*deg/*+0.5*360*deg/detNum2*/),
            ...
            G4RotationMatrix pqr = G4RotationMatrix();
            pqr.setRows(p,q,r);
            lg_rotm.rotateZ(i*360*deg/detNum2);
            lg_rotm.rotateX(180*deg);
            lg_rotm=pqr*lg_rotm;
            ...
            G4RotationMatrix delrin_rotm = G4RotationMatrix();
            G4ThreeVector w = G4ThreeVector(sin(theta2+90*deg)*sin(i*360*deg/detNum2+.001*deg), sin(theta2+90*deg)*cos(i*360*deg/detNum2+.001*deg), -cos(theta2+90*deg));
            ...
            G4RotationMatrix pqrdel = G4RotationMatrix();
            pqrdel.setRows(pdel,qdel,rdel);
            delrin_rotm.rotateZ((-i)*360*deg/detNum2+.001*deg);
            delrin_rotm=pqrdel*delrin_rotm;
    ...
    if(m_Ring3){
        ...
        G4double crystal_rmaxa3 = 0.0001*mm;
        ...
        for(int i = 1; i <= detNum3; i++)
        {
            ...
            G4ThreeVector d0 = G4ThreeVector(distanceFromTarget3*sin(coneangle3)*cos(0.5*360*deg/detNum3),
                                             distanceFromTarget3*sin(coneangle3)*sin(0.5*360*deg/detNum3),
                                             distanceFromTarget3*cos(coneangle3));
            G4ThreeVector df = G4ThreeVector(distanceFromTarget3*sin(theta3)*sin(i*360*deg/detNum3+.001*deg/*+0.5*360*deg/detNum3*/),
                                             distanceFromTarget3*sin(theta3)*cos(i*360*deg/detNum3+.001*deg/*+0.5*360*deg/detNum3*/),
            ...
            G4RotationMatrix pqr = G4RotationMatrix();
            pqr.setRows(p,q,r);
            lg_rotm.rotateZ(i*360*deg/detNum3);
            lg_rotm.rotateX(180*deg);
            lg_rotm=pqr*lg_rotm;
            G4ThreeVector lg_pos = (distanceFromTarget3+detThickness3+0.5*lgThickness3)/df.mag()*df;
            G4Transform3D lg_trans = G4Transform3D(lg_rotm, lg_pos);
            G4RotationMatrix delrin_rotm = G4RotationMatrix();
            G4ThreeVector w = G4ThreeVector(sin(theta3+90*deg)*sin(i*360*deg/detNum3+.001*deg), sin(theta3+90*deg)*cos(i*360*deg/detNum3+.001*deg), -cos(theta3+90*deg));
            ...
            G4RotationMatrix pqrdel = G4RotationMatrix();
            pqrdel.setRows(pdel,qdel,rdel);
            delrin_rotm.rotateZ((-i)*360*deg/detNum3+.001*deg);
            delrin_rotm=pqrdel*delrin_rotm;
    ...
    if(m_Ring4){
        ...
        G4double crystal_rmaxa4 = 0.0001*mm;
        ...
        for(int i = 1; i <= detNum4; i++)
        {
            ...
            G4ThreeVector d0 = G4ThreeVector(distanceFromTarget4*sin(coneangle4)*cos(0.5*360*deg/detNum4),
                                             distanceFromTarget4*sin(coneangle4)*sin(0.5*360*deg/detNum4),
                                             distanceFromTarget4*cos(coneangle4));
            G4ThreeVector df = G4ThreeVector(distanceFromTarget4*sin(theta4)*sin(i*360*deg/detNum4+.001*deg/*+0.5*360*deg/detNum4*/),
                                             distanceFromTarget4*sin(theta4)*cos(i*360*deg/detNum4+.001*deg/*+0.5*360*deg/detNum4*/),
            ...
            G4RotationMatrix pqr = G4RotationMatrix();
            pqr.setRows(p,q,r);
            lg_rotm.rotateZ(i*360*deg/detNum4);
            lg_rotm.rotateX(180*deg);
            lg_rotm=pqr*lg_rotm;
            G4ThreeVector lg_pos = (distanceFromTarget4+detThickness4+0.5*lgThickness4)/df.mag()*df;
            G4Transform3D lg_trans = G4Transform3D(lg_rotm, lg_pos);
            G4RotationMatrix delrin_rotm = G4RotationMatrix();
            G4ThreeVector w = G4ThreeVector(sin(theta4+90*deg)*sin(i*360*deg/detNum4+.001*deg), sin(theta4+90*deg)*cos(i*360*deg/detNum4+.001*deg), -cos(theta4+90*deg));
            ...
            G4RotationMatrix pqrdel = G4RotationMatrix();
            pqrdel.setRows(pdel,qdel,rdel);
            delrin_rotm.rotateZ((-i)*360*deg/detNum4+.001*deg);
            delrin_rotm=pqrdel*delrin_rotm;
            G4ThreeVector delrin_pos = (distanceFromTarget4+detThickness4+lgThickness4+delrinA4/2+.0003*mm)/df.mag()*df;  //the .0003 mm is a fudge factor to prevent overlapping
            G4Transform3D delrin_trans = G4Transform3D(delrin_rotm, delrin_pos);
            //if(i != 1){
            bool placeCrystal = true;
            for (unsigned int i2=0; i2 < m_Array.size(); i2++){
    ...
    if(m_Ring5){
        ...
        G4double crystal_rmaxa5 = 0.0001*mm;
        ...
        for(int i = 1; i <= detNum5; i++)
        {
            ...
            G4ThreeVector d0 = G4ThreeVector(distanceFromTarget5*sin(coneangle5)*cos(0.5*360*deg/detNum5),
                                             distanceFromTarget5*sin(coneangle5)*sin(0.5*360*deg/detNum5),
                                             distanceFromTarget5*cos(coneangle5));
            G4ThreeVector df = G4ThreeVector(distanceFromTarget5*sin(theta5)*sin(i*360*deg/detNum5+.001*deg/*+0.5*360*deg/detNum5*/),
                                             distanceFromTarget5*sin(theta5)*cos(i*360*deg/detNum5+.001*deg/*+0.5*360*deg/detNum5*/),
            ...
            G4RotationMatrix pqr = G4RotationMatrix();
            pqr.setRows(p,q,r);
            lg_rotm.rotateZ(i*360*deg/detNum5);
            lg_rotm.rotateX(180*deg);
            lg_rotm=pqr*lg_rotm;
            G4ThreeVector lg_pos = (distanceFromTarget5+detThickness5+0.5*lgThickness5)/df.mag()*df;
            G4Transform3D lg_trans = G4Transform3D(lg_rotm, lg_pos);
            G4RotationMatrix delrin_rotm = G4RotationMatrix();
            G4ThreeVector w = G4ThreeVector(sin(theta5+90*deg)*sin(i*360*deg/detNum5+.001*deg), sin(theta5+90*deg)*cos(i*360*deg/detNum5+.001*deg), -cos(theta5+90*deg));
            ...
            G4RotationMatrix pqrdel = G4RotationMatrix();
            pqrdel.setRows(pdel,qdel,rdel);
            delrin_rotm.rotateZ((-i)*360*deg/detNum5+.001*deg);
            delrin_rotm=pqrdel*delrin_rotm;
            G4ThreeVector delrin_pos = (distanceFromTarget5+detThickness5+lgThickness5+delrinA5/2+.0003*mm)/df.mag()*df;  //the .0003 mm is a fudge factor to prevent overlapping
            G4Transform3D delrin_trans = G4Transform3D(delrin_rotm, delrin_pos);
            //if(i != 1){
            bool placeCrystal = true;
            for (unsigned int i2=0; i2 < m_Array.size(); i2++){
    ...
    if(m_Ring6){
        ...
        for(int i = 1; i <= detNum6; i++)
        {
            ...
            G4ThreeVector df6 = G4ThreeVector(cos(i*360*deg/detNum6+0.5*360*deg/detNum6),
                                              sin(i*360*deg/detNum6+0.5*360*deg/detNum6),
                                              0);
            ...
            G4RotationMatrix abc6 = G4RotationMatrix();
            abc6.setRows(a6,b6,c6);
            crystal_rotm.rotateZ(i*360*deg/detNum6+0.5*360*deg/detNum6);
            crystal_rotm=abc6*crystal_rotm;
            G4ThreeVector crystal_pos = G4ThreeVector((distanceFromTarget6+0.5*detThickness6)*cos(i*360*deg/detNum6+0.5*360*deg/detNum6),
                                                      (distanceFromTarget6+0.5*detThickness6)*sin(i*360*deg/detNum6+0.5*360*deg/detNum6),
                                                      0)+delrin_pos6;
            G4Transform3D crystal_trans = G4Transform3D(crystal_rotm, crystal_pos);
            G4RotationMatrix lg_rotm = G4RotationMatrix();
            G4ThreeVector df = G4ThreeVector(distanceFromTarget6*sin(theta6)*sin(i*360*deg/detNum6/*+0.5*360*deg/detNum6*/),
                                             distanceFromTarget6*sin(theta6)*cos(i*360*deg/detNum6/*+0.5*360*deg/detNum6*/),
            ...
            G4RotationMatrix pqr = G4RotationMatrix();
            pqr.setRows(p,q,r);
            lg_rotm.rotateZ(i*360*deg/detNum6);
            lg_rotm.rotateX(180*deg);
            lg_rotm=pqr*lg_rotm;
            G4ThreeVector lg_pos = (distanceFromTarget6+detThickness6+0.5*lgThickness6)/df.mag()*df;
            G4Transform3D lg_trans = G4Transform3D(lg_rotm, lg_pos);
            G4RotationMatrix delrin_rotm = G4RotationMatrix();
            G4ThreeVector w = G4ThreeVector(sin(theta6+90*deg)*sin(i*360*deg/detNum6+.001*deg), sin(theta6+90*deg)*cos(i*360*deg/detNum6+.001*deg), -cos(theta6+90*deg));
            G4ThreeVector delrin_Axis = w.cross(-z6);
            delrin_Axis = delrin_Axis/delrin_Axis.mag();
            G4double udx = delrin_Axis.x();
            G4double udy = delrin_Axis.y();
            G4double udz = delrin_Axis.z();
            G4double delAng = 90*deg-theta6;
            G4ThreeVector pdel = G4ThreeVector(cos(delAng)+pow(udx,2)*(1-cos(delAng)), udx*udy*(1-cos(delAng))-udz*sin(delAng), udx*udz*(1-cos(delAng))+udy*sin(delAng));
            G4ThreeVector qdel = G4ThreeVector(udy*udx*(1-cos(delAng))+udz*sin(delAng), cos(delAng)+pow(udy,2)*(1-cos(delAng)), udy*udz*(1-cos(delAng))-udx*sin(delAng));
            G4ThreeVector rdel = G4ThreeVector(udz*udx*(1-cos(delAng))-udy*sin(delAng), udz*udy*(1-cos(delAng))+udx*sin(delAng), cos(delAng)+pow(udz,2)*(1-cos(delAng)));
            G4RotationMatrix pqrdel = G4RotationMatrix();
            pqrdel.setRows(pdel,qdel,rdel);
            delrin_rotm.rotateZ((-i)*360*deg/detNum6+.001*deg);
            delrin_rotm=pqrdel*delrin_rotm;
            G4ThreeVector delrin_pos = (distanceFromTarget6+detThickness6+lgThickness6+delrinA6/2+.0003*mm)/df.mag()*df;  //the .0003 mm is a fudge factor to prevent overlapping
            G4Transform3D delrin_trans = G4Transform3D(delrin_rotm, delrin_pos);
            bool placeCrystal = true;
            for (unsigned int i2=0; i2 < m_Array.size(); i2++){
                if (m_Array[i2] == 54+i){
    ...
    if(m_Ring7){
        ...
        G4double crystal_rmaxa7 = 0.0001*mm;
        ...
        for(int i = 1; i <= detNum7; i++)
        {
            ...
            G4ThreeVector d0 = G4ThreeVector(distanceFromTarget7*sin(coneangle7)*cos(0.5*360*deg/detNum7),
                                             distanceFromTarget7*sin(coneangle7)*sin(0.5*360*deg/detNum7),
                                             distanceFromTarget7*cos(coneangle7));
            G4ThreeVector df = G4ThreeVector(distanceFromTarget7*sin(theta7)*sin(i*360*deg/detNum7+.001*deg/*+0.5*360*deg/detNum7*/),
                                             distanceFromTarget7*sin(theta7)*cos(i*360*deg/detNum7+.001*deg/*+0.5*360*deg/detNum7*/),
            ...
            G4RotationMatrix pqr = G4RotationMatrix();
            pqr.setRows(p,q,r);
            lg_rotm.rotateZ(i*360*deg/detNum7);
            lg_rotm.rotateX(180*deg);
            lg_rotm=pqr*lg_rotm;
            G4ThreeVector lg_pos = (distanceFromTarget7+detThickness7+0.5*lgThickness7+.0003*mm)/df.mag()*df;
            G4Transform3D lg_trans = G4Transform3D(lg_rotm, lg_pos);
            G4RotationMatrix delrin_rotm = G4RotationMatrix();
            G4ThreeVector w = G4ThreeVector(sin(theta7+90*deg)*sin(i*360*deg/detNum7+.001*deg), sin(theta7+90*deg)*cos(i*360*deg/detNum7+.001*deg), -cos(theta7+90*deg));
            ...
            G4RotationMatrix pqrdel = G4RotationMatrix();
            pqrdel.setRows(pdel,qdel,rdel);
            delrin_rotm.rotateZ((i)*360*deg/detNum7+.001*deg);
            delrin_rotm.rotateX(180*deg);
            delrin_rotm=pqrdel*delrin_rotm;
            G4ThreeVector delrin_pos = (distanceFromTarget7+detThickness7+lgThickness7+delrinA7/2+.0003*mm)/df.mag()*df;  //the .0003 mm is a fudge factor to prevent overlapping
            G4Transform3D delrin_trans = G4Transform3D(delrin_rotm, delrin_pos);
            bool placeCrystal = true;
            bool placeDel = true;
            for (unsigned int i2=0; i2 < m_Array.size(); i2++){
                if (148-m_Array[i2] == 68+i){
                    placeCrystal = false;
                }
                if (m_Array[i2] == 68+i){
                    placeDel = false;
    ...
    if(m_Ring8) {
        ...
        G4double crystal_rmaxa8 = 0.0001*mm;
        ...
        for(int i = 1; i <= detNum8; i++)
        {
            ...
            G4ThreeVector d0 = G4ThreeVector(distanceFromTarget8*sin(coneangle8)*cos(0.5*360*deg/detNum8),
                                             distanceFromTarget8*sin(coneangle8)*sin(0.5*360*deg/detNum8),
                                             distanceFromTarget8*cos(coneangle8));
            G4ThreeVector df = G4ThreeVector(distanceFromTarget8*sin(theta8)*sin(i*360*deg/detNum8+.001*deg/*+0.5*360*deg/detNum8*/),
                                             distanceFromTarget8*sin(theta8)*cos(i*360*deg/detNum8+.001*deg/*+0.5*360*deg/detNum8*/),
            ...
            G4RotationMatrix pqr = G4RotationMatrix();
            pqr.setRows(p,q,r);
            lg_rotm.rotateZ(i*360*deg/detNum8);
            lg_rotm.rotateX(180*deg);
            lg_rotm=pqr*lg_rotm;
            G4ThreeVector lg_pos = (distanceFromTarget8+detThickness8+0.5*lgThickness8+.0003*mm)/df.mag()*df;
            G4Transform3D lg_trans = G4Transform3D(lg_rotm, lg_pos);
            G4RotationMatrix delrin_rotm = G4RotationMatrix();
            G4ThreeVector w = G4ThreeVector(sin(theta8+90*deg)*sin(i*360*deg/detNum8+.001*deg), sin(theta8+90*deg)*cos(i*360*deg/detNum8+.001*deg), -cos(theta8+90*deg));
            ...
            G4RotationMatrix pqrdel = G4RotationMatrix();
            pqrdel.setRows(pdel,qdel,rdel);
            delrin_rotm.rotateZ((i)*360*deg/detNum8+.001*deg);
            delrin_rotm.rotateX(180*deg);
            delrin_rotm=pqrdel*delrin_rotm;
            G4ThreeVector delrin_pos = (distanceFromTarget8+detThickness8+lgThickness8+delrinA8/2+.0003*mm)/df.mag()*df;  //the .0003 mm is a fudge factor to prevent overlapping
            G4Transform3D delrin_trans = G4Transform3D(delrin_rotm, delrin_pos);
            bool placeCrystal = true;
            bool placeDel = true;
            for (unsigned int i2=0; i2 < m_Array.size(); i2++){
                if (170-m_Array[i2] == 80+i){
                    placeCrystal = false;
                }
                if (m_Array[i2] == 80+i){
                    placeDel = false;
    ...
    if(m_Ring9){
        ...
        G4double crystal_rmaxa9 = 0.0001*mm;
        ...
        for(int i = 1; i <= detNum9; i++)
        {
            ...
            G4ThreeVector d0 = G4ThreeVector(distanceFromTarget9*sin(coneangle9)*cos(0.5*360*deg/detNum9), 
                                             distanceFromTarget9*sin(coneangle9)*sin(0.5*360*deg/detNum9), 
                                             distanceFromTarget9*cos(coneangle9));
            G4ThreeVector df = G4ThreeVector(distanceFromTarget9*sin(theta9)*sin(i*360*deg/detNum9+.001*deg/*+0.5*360*deg/detNum9*/), 
                                             distanceFromTarget9*sin(theta9)*cos(i*360*deg/detNum9+.001*deg/*+0.5*360*deg/detNum9*/), 
            ...
            G4RotationMatrix lg_rotm = G4RotationMatrix();
            /*
             lg_rotm.rotateZ(i*360*deg/detNum9);
            ...
            G4Transform3D lg_trans = G4Transform3D(lg_rotm, lg_pos);
            G4RotationMatrix delrin_rotm = G4RotationMatrix();
            G4ThreeVector w = G4ThreeVector(sin(theta9+90*deg)*sin(i*360*deg/detNum9+.001*deg), sin(theta9+90*deg)*cos(i*360*deg/detNum9+.001*deg), -cos(theta9+90*deg));
            ...
            G4RotationMatrix pqrdel = G4RotationMatrix();
            pqrdel.setRows(pdel,qdel,rdel);
            delrin_rotm.rotateZ((i)*360*deg/detNum9+.001*deg);
            delrin_rotm.rotateX(180*deg);
            delrin_rotm=pqrdel*delrin_rotm;
            G4ThreeVector delrin_pos = (distanceFromTarget9+detThickness9+lgThickness9+delrinA9/2+.0003*mm)/df.mag()*df;  //the .0003 mm is a fudge factor to prevent overlapping
            G4Transform3D delrin_trans = G4Transform3D(delrin_rotm, delrin_pos); 
            bool placeCrystal = true;
            bool placeDel = true;
            for (unsigned int i2=0; i2 < m_Array.size(); i2++){
                if (186-m_Array[i2] == 90+i){ 
                    placeCrystal = false;
                }
                if (m_Array[i2] == 90+i){
                    placeDel = false;
