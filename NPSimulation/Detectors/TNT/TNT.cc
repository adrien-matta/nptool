...
G4LogicalVolume* TNT::BuildSquareDetector(){
  if(!m_SquareDetector){
    G4Box* box = new G4Box("TNT_Box",TNT_NS::Width*0.5,
        TNT_NS::Width*0.5,TNT_NS::Thickness*0.5);
...
G4LogicalVolume* TNT::BuildCylindricalDetector(){
  if(!m_CylindricalDetector){
    G4Tubs* tub = new G4Tubs("TNT_Cyl",0,TNT_NS::Radius,TNT_NS::Thickness*0.5,0,360*deg);
...
void TNT::ConstructDetector(G4LogicalVolume* world){
  for (unsigned short i = 0 ; i < m_R.size() ; i++) {
    ...
    Det_pos+=Det_pos.unit()*TNT_NS::Thickness*0.5;
...
void TNT::ReadSensitive(const G4Event* event){
  ...
  for (Calo_itr = CaloHitMap->GetMap()->begin() ; Calo_itr != CaloHitMap->GetMap()->end() ; Calo_itr++){
    ...
    double Energy = RandGauss::shoot(Info[0],TNT_NS::ResoEnergy);
    if(Energy>TNT_NS::EnergyThreshold){
      double Time = RandGauss::shoot(Info[1],TNT_NS::ResoTime);
