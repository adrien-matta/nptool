  ...
  for(unsigned int i = 0 ; i < blocks.size() ; i++){
    if(blocks[i]->HasTokenList(token)){
      ...
      for(unsigned int j = 0 ; j < particleName.size() ; j++){
        ...
        else if(particleName[j]=="e-") {it->m_particleName.push_back("e-") ;}
    ...
    for(unsigned int p=0; p<par.m_particleName.size(); p++){
      for(int i=0; i<par.m_Multiplicty[p]; i++){
        ...
        if(par.m_particle==NULL){
          ...
          if(par.m_particleName[p]=="gamma" || par.m_particleName[p]=="neutron" ||  par.m_particleName[p]=="opticalphoton" || par.m_particleName[p]=="e-" ){
