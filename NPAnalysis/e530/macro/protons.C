...
void protons(const char* frun = "60Fe_AnalysedData")
{
  ...
  */
  ...
  TH2F* E_Theta       = new TH2F("E vs theta tot","E vs theta",180,110,170,5000,0,25000);
  TH2F* E_Theta1      = new TH2F("E vs theta EDE","E vs theta EDE",180,110,170,5000,0,25000);
  TH2F* E_Theta2      = new TH2F("E vs theta ETof","E vs theta ETof",180,110,170,5000,0,25000);
  ...
  TH2F* DE_TOF_protons= new TH2F("E vs TOF protons","E vs TOF protons",120,360,400,500,0,10000);
  ...
  for (int i = 0; i < nentries; i++) {
    ...
    tree ->GetEntry(i);
     ...
     if(cats->PositionOnTargetX < 12 && cats->PositionOnTargetX > -12 && cats->PositionOnTargetY > -12 && cats->PositionOnTargetY < 12)
      { 
	      ...
	      if(M2 -> TelescopeNumber[hit] !=4 ) 
		{
		  ...
		  if(cut_protons -> IsInside(M2 -> SiLi_E[hit], M2 -> Si_E[hit])) 
		    {
		      ...
		      E_Theta        -> Fill(Theta*180/3.14,E);
		      E_Theta1       -> Fill(Theta*180/3.14,E);
		  ...
		  else 
		  {
			...
			if(cut_protons_E_TOF1 -> IsInside(M2 -> Si_T[hit], M2 -> Si_E[hit]) )
			  {
			    // DE_TOF_protons -> Fill(M2 -> Si_T[hit], M2 -> Si_E[hit]);
			    E_Theta        -> Fill(Theta*180/3.14,E);
			    E_Theta2       -> Fill(Theta*180/3.14,E);
  ...
  TCanvas *c2 = new TCanvas("c2","DE_TOF",900,900);
  c2->Divide(2,2);
  c2->cd(1);
  DE_TOF->Draw("colz");
  c2->cd(2);
  DE_TOF_protons->Draw("colz");
